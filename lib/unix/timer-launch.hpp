...
//
#ifndef TIMER_LAUNCH_HPP
#define TIMER_LAUNCH_HPP
...
namespace unix
{
  ...
  inline timer_launch_t::timer_launch_t (bool fground,
                                         start_t start_function,
                                         stop_t stop_function,
                                         asio::io_context &context,
  ...
  inline bool timer_launch_t::start ()
  {
  ...
  inline bool timer_launch_t::background ()
  {
    daemon_retval_init ();
    pid_t pid = 0;
  ...
  void timer_launch_t::stop (bool pid_file, bool stop_callback)
  {
    log_t::info ("Exiting...");
  ...
  bool timer_launch_t::signal_init ()
  {
    bool status = true;
  ...
  void timer_launch_t::check_signal (const asio::error_code &error)
  {
    if (error) {
      // asio error ?
      stop (true, true);
    ...
    if (need_exit () == false) {
      ...
      m_timer.async_wait
        (std::bind
         (&timer_launch_t::check_signal, this, std::placeholders::_1));
    } else
      stop (true, true);
  }
